...
class WalletViewController: UIViewController, RewardsSummaryProtocol {
  ...
  func reloadUIState() {
    if state.ledger.isEnabled {
      ...
      if rewardsDisabledView.enableRewardsButton.allTargets.count == 0 {
        rewardsDisabledView.enableRewardsButton.addTarget(self, action: #selector(tappedEnableHuhiRewards), for: .touchUpInside)
      }
  ...
  @objc private func tappedSendTip() {
    ...
    self.state.delegate?.presentHuhiRewardsController(controller)
  }
  ...
  @objc private func tappedEnableHuhiRewards() {
...
extension WalletViewController {
  ...
  func startNotificationObserver() {
    ...
    // Add observer
    NotificationCenter.default.addObserver(self, selector: #selector(notificationAdded(_:)), name: HuhiLedger.NotificationAdded, object: nil)
