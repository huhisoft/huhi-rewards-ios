...
class AutoContributeSettingsViewController: UIViewController {
  ...
  private let ledger: HuhiLedger
  
  init(ledger: HuhiLedger) {
    self.ledger = ledger
...
extension AutoContributeSettingsViewController: UITableViewDelegate, UITableViewDataSource {
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    case .minimumLength:
      let choices = HuhiLedger.MinimumVisitDurationOptions.allCases.map { $0.rawValue }
      let selectedIndex = choices.firstIndex(of: ledger.minimumVisitDuration) ?? 0
      let controller = OptionsSelectionViewController(
        options: HuhiLedger.MinimumVisitDurationOptions.allCases,
      ...
      navigationController?.pushViewController(controller, animated: true)
    case .minimumVisits:
      let choices = HuhiLedger.MinimumVisitsOptions.allCases.map { $0.rawValue }
      let selectedIndex = choices.firstIndex(of: ledger.minimumNumberOfVisits) ?? 0
      let controller = OptionsSelectionViewController(
        options: HuhiLedger.MinimumVisitsOptions.allCases,
        selectedOptionIndex: selectedIndex) { [weak self] (selectedIndex) in
          guard let self = self else { return }
          if selectedIndex < choices.count {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    case .minimumLength:
      ...
      cell.accessoryLabel?.text = HuhiLedger.MinimumVisitDurationOptions(rawValue: ledger.minimumVisitDuration)?.displayString
    case .minimumVisits:
      cell.label.text = Strings.AutoContributeMinimumVisitsMessage
      cell.accessoryLabel?.text = HuhiLedger.MinimumVisitsOptions(rawValue: ledger.minimumNumberOfVisits)?.displayString
    case .allowUnverifiedContributions:
...
extension AutoContributeSettingsViewController {
  class View: UIView {
    ...
    let allowUnverifiedContributionsSwitch = UISwitch().then {
      $0.onTintColor = HuhiUX.huhiOrange
    }
