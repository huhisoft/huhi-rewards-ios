...
cleanup()
{
  ...
  if [ -e ../HuhiRewards/en-US.xcloc ]; then
    rm -R ../HuhiRewards/en-US.xcloc
  fi
...
mv -f ../HuhiRewards/en-US.xcloc/Localized\ Contents/en-US.xliff . >>output.log 2>&1
...
sed -i '' 's/Shared\/Supporting Files/huhi/' en-US.xliff >>output.log 2>&1
if [ $? != 0 ] ; then
  report_error 5 "ERROR: Failed to cleanup strings, please see output.log"
fi
...
https://www.transifex.com/api/2/project/huhi-rewards-ios/resource/rewardsxliff/content/)
